...
func generate_buildings_in_chunk(chunk_position):
	...
	for x in range(CHUNK_SIZE):
		for z in range(CHUNK_SIZE):
			...
			add_child(t)
			t.global_transform.origin.x = chunk_position.x * CHUNK_SIZE * TILE_SIZE + x * TILE_SIZE
			t.global_transform.origin.z = chunk_position.y * CHUNK_SIZE * TILE_SIZE + z * TILE_SIZE
			if x == 0 and z == 0:
